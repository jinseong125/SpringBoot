...
@SpringBootTest
@TestInstance(Lifecycle.PER_CLASS)  //----- 인스턴스(객체)를 한 클래스당 한개만 생성합니다.
class ApplicationTests {
  ...
  public void deleteBoardTest() {
    ...
    /*
     *  Optional이란?
     *  null을 안전하게 다루기 위한 컨테이너 클래스 값이 있으면 그 값을 감싸고, 없으면 Optional.empty()로 표현.
     *  NullPointerException(NPE)을 예방하고, 명시적으로 값이 없음을 처리하게 해줌.
     */
    Optional<Board> found = boardRedisRepository.findById(bid);
    Assertions.assertFalse(found.isPresent());  //----- isPresent : Optional 안에 값이 존재하는지 여부를 확인하는 메소드.반환 타입은 boolean.
  }
